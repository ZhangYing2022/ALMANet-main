...
class REClassifier(nn.Module):
    ...
    def forward(self, input_ids=None, output_state=None, output_attention=None):
        (output_state, vision_hidden_states, text_hidden_states, visual_only) = output_state
        ...
        visual_hidden = torch.Tensor(bsz, 2 * hidden_size)  # batch, 2*hidden
        text_hidden = torch.Tensor(bsz, 2 * hidden_size)  # batch, 2*hidden
        entity_attention = torch.Tensor(bsz, 13, 12, 2, 61)  # batch, 1, seq_len
        ...
        for i in range(bsz):
            ...
            #visual for contribution
            visual_only_head = visual_only[i, head_idx, :].squeeze()
            visual_only_tail = visual_only[i, tail_idx, :].squeeze()
            visual_hidden[i] = torch.cat([visual_only_head, visual_only_tail], dim=-1)
            #text for contribution
            text_only_head = text_hidden_states[i, head_idx, :].squeeze()
            text_only_tail = text_hidden_states[i, tail_idx, :].squeeze()
            text_hidden[i] = torch.cat([text_only_head, text_only_tail], dim=-1)
            # entity attention matrix
            #entity_attention[i] = output_attention[i, :, :, [head_idx, tail_idx], :]


        if torch.cuda.is_available():
            entity_hidden_state = entity_hidden_state.to('cuda')
            visual_hidden = visual_hidden.to('cuda')
            text_hidden = text_hidden.to('cuda')
        logits = self.classifier(entity_hidden_state)
        logits_visual = self.classifier(visual_hidden)
        logits_text = self.classifier(text_hidden)

        return logits, logits_visual, logits_text, visual_hidden, text_hidden
...
class BertVitInterReModel(nn.Module):
    ...
    def forward(
        ...
        if task == 're':
            (output_state, vision_hidden_states, text_hidden_states, visual_only) = output
            output_attention = output_state.attentions

            bert_vit_logits, logits_v, logits_t, visual_feature, text_feature = self.re_classifier(output_state=output, input_ids=input_ids, output_attention=output_attention)

            if labels is not None:
                label_ce_loss_fn = nn.CrossEntropyLoss()
                label_loss_bert_vit = label_ce_loss_fn(bert_vit_logits, labels.view(-1))

                return label_loss_bert_vit, bert_vit_logits, (logits_v, logits_t, visual_feature, text_feature)
