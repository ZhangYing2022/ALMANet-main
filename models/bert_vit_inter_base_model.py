...
class Vision2TextAttention(nn.Module):
    ...
    def forward(
        ...
        if output_attentions:
            ...
            #attn_weights_reshaped = attn_weights_reshaped.mean(dim=1).view(bsz, tgt_len, src_tgt_len)
        else:
            attn_weights_reshaped = None
...
class VisualLayerWiseMoE(nn.Module):
    def __init__(self, config, aggred_layer_num=1):
        ...
        self.experts1_t = nn.Parameter(torch.Tensor(self.n_experts + 1, config.hidden_size, config.hidden_size),
                                       requires_grad=True)
        torch.nn.init.xavier_uniform(self.experts1_t.data)
        self.activation_fn = ACT2FN[config.hidden_act]
        self.experts2 = nn.Parameter(torch.Tensor(self.n_experts, config.hidden_size, config.hidden_size),
                                     requires_grad=True)
        self.experts2_t = nn.Parameter(torch.Tensor(self.n_experts-1, config.hidden_size, config.hidden_size),
                                     requires_grad=True)
        torch.nn.init.xavier_uniform(self.experts2.data)
        torch.nn.init.xavier_uniform(self.experts2_t.data)
        self.expert_router = nn.Linear((1 + self.n_experts) * config.hidden_size, self.n_experts + 1)
        self.expert_router_v = nn.Linear(self.n_experts * config.hidden_size, self.n_experts)
        self.expert_router_t = nn.Linear(4 * config.hidden_size, self.n_experts+1)

    def forward(self,
                text_hidden_states: torch.Tensor = None,
                text_last4_hidden_states: Tuple = None,
        ...
        #text 专家组
        text_expert_weights = torch.cat(text_last4_hidden_states, dim=-1)
        text_expert_weights = self.expert_router_t(text_expert_weights)
        text_expert_weights = torch.softmax(text_expert_weights, dim=-1)
        text_last4_hidden_states = torch.stack(text_last4_hidden_states, dim=2)
        text_out = torch.einsum('bsni,nio->bsno', text_last4_hidden_states, self.experts1_t)
        text_out = self.activation_fn(text_out)
        text_out = torch.einsum('bsno,bsn->bsno', text_out, text_expert_weights)

        #9.18 获取加权视觉特征
        visual_for_contribution = expert_router_input[:, :, -3 * embed_dim:]
        visual_weights = torch.softmax(self.expert_router_v(visual_for_contribution), dim=-1) # bsz, 3
        visual_for_contribution = torch.einsum('bsno,bsn->bsno', layer_wise_visual_hidden_states, visual_weights)
        visual_for_contribution = torch.sum(visual_for_contribution, dim=2)
        ...
        text_fusion = torch.sum(text_out, dim=2).unsqueeze(2)
        moe_fusion = torch.sum(experts_out, dim=2).unsqueeze(2)
        moe_fusion = torch.cat([text_fusion, moe_fusion], dim=2)
        moe_fusion = torch.einsum('bsno,nio->bsni', moe_fusion, self.experts2_t)
        moe_fusion = torch.sum(moe_fusion, dim=2)
        return moe_fusion, visual_for_contribution
...
class Vision2TextLayerWiseAgg(nn.Module):
    ...
    def forward(
        ...
        text_last4_hidden_states = all_text_hidden_states[-4:]
        ...
        cross_modal_hidden_state, visual_for_contribution = self.moe_layer_wise_agg(text_hidden_states=text_hidden_states,
                                                                                    text_last4_hidden_states=text_last4_hidden_states,
        ...
        return outputs, visual_for_contribution
...
class BertVitInterEncoder(nn.Module):
    ...
    def forward(
        ...
        vision2text_outputs, visual_only = self.vision2text_layer_agg(all_text_hidden_states=all_text_hidden_states,
        ...
        ), vision_hidden_states, text_hidden_states, visual_only
...
class BertVitInterBaseModel(nn.Module):
    ...
    def forward(
        ...
        encoder_outputs, vision_hidden_states, text_hidden_states, visual_only = self.encoder(
